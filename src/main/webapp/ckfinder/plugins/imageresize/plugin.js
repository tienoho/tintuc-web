...
CKFinder.addPlugin( 'imageresize', {
	...
	connectorInitialized : function( api, xml )
	{
        ...
        node = xml.selectSingleNode( 'Connector/PluginsInfo/imageresize/@w104' );
        if ( node )
            CKFinder.config.imageresize_w104 = node.value;
        node = xml.selectSingleNode( 'Connector/PluginsInfo/imageresize/@w150' );
        if ( node )
            CKFinder.config.imageresize_w150 = node.value;
        node = xml.selectSingleNode( 'Connector/PluginsInfo/imageresize/@w214' );
        if ( node )
            CKFinder.config.imageresize_w214 = node.value;
        node = xml.selectSingleNode( 'Connector/PluginsInfo/imageresize/@w300' );
        if ( node )
            CKFinder.config.imageresize_w300 = node.value;
        node = xml.selectSingleNode( 'Connector/PluginsInfo/imageresize/@w336' );
        if ( node )
            CKFinder.config.imageresize_w336 = node.value;
        node = xml.selectSingleNode( 'Connector/PluginsInfo/imageresize/@w702' );
        if ( node )
            CKFinder.config.imageresize_w702 = node.value;
        node = xml.selectSingleNode( 'Connector/PluginsInfo/imageresize/@w1000' );
        if ( node )
            CKFinder.config.imageresize_w1000 = node.value;
	},
			...
			return {
				...
				onShow : function()
				{
					var dialog = this,
                        ...
                        thumbw104 = CKFinder.config.imageresize_w104,
                        thumbw150 = CKFinder.config.imageresize_w150,
                        thumbw214 = CKFinder.config.imageresize_w214,
                        thumbw300 = CKFinder.config.imageresize_w300,
                        thumbw336 = CKFinder.config.imageresize_w336,
                        thumbw702 = CKFinder.config.imageresize_w702,
                        thumbw1000 = CKFinder.config.imageresize_w1000;
					...
					api.connector.sendCommand( 'ImageResizeInfo', {
						...
						function( xml )
						{
							...
							if ( width && height )
							{
                                ...
                                checkThumbs( 'w104', thumbw104 );
                                checkThumbs( 'w150', thumbw150 );
                                checkThumbs( 'w214', thumbw214 );
                                checkThumbs( 'w300', thumbw300 );
                                checkThumbs( 'w336', thumbw336 );
                                checkThumbs( 'w702', thumbw702 );
                                checkThumbs( 'w1000', thumbw1000 );
							}
						},
						file.folder.type,
						file.folder
                    ...
                    if ( !thumbw104 )
                        dialog.getContentElement( 'tab1', 'w104' ).getElement().hide();
                    if ( !thumbw150 )
                        dialog.getContentElement( 'tab1', 'w150' ).getElement().hide();
                    if ( !thumbw214 )
                        dialog.getContentElement( 'tab1', 'w214' ).getElement().hide();
                    if ( !thumbw300 )
                        dialog.getContentElement( 'tab1', 'w300' ).getElement().hide();
                    if ( !thumbw336 )
                        dialog.getContentElement( 'tab1', 'w336' ).getElement().hide();
                    if ( !thumbw702 )
                        dialog.getContentElement( 'tab1', 'w702' ).getElement().hide();
                    if ( !thumbw1000 )
                        dialog.getContentElement( 'tab1', 'w1000' ).getElement().hide();
				...
				onOk : function()
				{
					var dialog = this,
                        ...
                        w104 = dialog.getValueOf( 'tab1', 'w104' ),
                        w150 = dialog.getValueOf( 'tab1', 'w150' ),
                        w214 = dialog.getValueOf( 'tab1', 'w214' ),
                        w300 = dialog.getValueOf( 'tab1', 'w300' ),
                        w336 = dialog.getValueOf( 'tab1', 'w336' ),
                        w702 = dialog.getValueOf( 'tab1', 'w702' ),
                        w1000 = dialog.getValueOf( 'tab1', 'w1000' ),
								...
								[
									{
										...
										[
											...
											},
                                            {
                                                ...
                                                id : 'w104',
                                                checked : false,
                                                label : api.lang.Imageresize.thumbnailw104.replace( '%s', CKFinder.config.imageresize_w104 )
                                            },
                                            {
                                                type : 'checkbox',
                                                id : 'w150',
                                                checked : false,
                                                label : api.lang.Imageresize.thumbnailw150.replace( '%s', CKFinder.config.imageresize_w150 )
                                            },
                                            {
                                                type : 'checkbox',
                                                id : 'w214',
                                                checked : false,
                                                label : api.lang.Imageresize.thumbnailw214.replace( '%s', CKFinder.config.imageresize_w214 )
                                            },
                                            {
                                                type : 'checkbox',
                                                id : 'w300',
                                                checked : false,
                                                label : api.lang.Imageresize.thumbnailw300.replace( '%s', CKFinder.config.imageresize_w300 )
                                            },
                                            {
                                                type : 'checkbox',
                                                id : 'w336',
                                                checked : false,
                                                label : api.lang.Imageresize.thumbnailw336.replace( '%s', CKFinder.config.imageresize_w336 )
                                            },
                                            {
                                                type : 'checkbox',
                                                id : 'w702',
                                                checked : false,
                                                label : api.lang.Imageresize.thumbnailw702.replace( '%s', CKFinder.config.imageresize_w702 )
                                            },
                                            {
                                                type : 'checkbox',
                                                id : 'w1000',
                                                checked : false,
                                                label : api.lang.Imageresize.thumbnailw1000.replace( '%s', CKFinder.config.imageresize_w1000 )
                                            }
										]
									},
									{
